	...
	<div class="datable">
		<!-- <el-table :data="tableData.slice((currentPage-1)*PageSize,currentPage*PageSize)"  style="width:100%;">
			<el-table-column label="支付金额">
				...
				￥{{scope.row.payAmount | payAmount}}
				</template>
			</el-table-column>
			<el-table-column  label="退款金额">
				<template slot-scope="scope">
					￥{{scope.row.refundAmount | payAmount}}
				</template>
			</el-table-column>
			...
			<el-table-column label="支付订单号"   width='300'>
				<template slot-scope="scope">
					<span class="zz">支付</span>{{scope.row.payOrderId}}<br>
					<span class="qd">渠道</span>{{scope.row.channelPayOrderNo}}
				</template>
			</el-table-column>
			<el-table-column prop="tui" label="支付状态">
				<template slot-scope="scope">
		...
		<ul>
		  <li class="information li_vessel" v-for="(item,index) in tableData" :key="index"  @click="jinhao(item.payOrderId)">
			<p>
				<strong>应用APPID：</strong>{{item.appId}}
			</p>
			<p>
				<strong>支付金额：</strong>￥{{item.payAmount | payAmount}}
			</p>
			<p>
		...
		<!-- 蒙尘 -->
		<el-dialog title="退款订单详情" append-to-body :visible.sync="dialogVisible" width="98%">
			<div class="chen">
				<p>
				...
				<p>
					<span>退款订单号： <br> <br><span class="dingdan">{{dilogoList.refundOrderId}}</span></span>
				</p>
				<p>
				...
				<p>
					<span>支付金额：<span class="zhifu">￥{{dilogoList.payAmount | payAmount}}</span></span>
				</p>
				<p>
					<span>退款金额： <span class="zhifu">￥{{dilogoList.refundAmount | payAmount}}</span></span>
				</p>
...
<script>
	import {refundOrder,refundOrderinfo} from '@/utils/ordercenter.js'
	export default{
		filters:{
			payAmount(e){
				return e/100

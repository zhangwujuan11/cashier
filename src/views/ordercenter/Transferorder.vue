					...
					<el-form :model="myFrom"  status-icon  label-width="100px" class="demo-ruleForm">
					  <el-form-item  prop="pass">
					   <div class="serboxx">
					   	...
					   	<el-input v-model="query"  @keyup.enter.native="serchdata()" placeholder="输入应用APPID或转账/商户/渠道订单号">
...
<script>
	...
	import Tabledata from '@/components/ordercenter/Transfertable.vue'
	import Footn from '@/components/Footn.vue'
	import {transferOrders} from '@/utils/ordercenter.js'
  export default {
    data() {
      return {
		options: [{
    ...
    methods:{
    	...
    	// 查询
    	serchdata(){
    		transferOrders({'query':this.query}).then(res=>{
	...
	watch:{
		...
		myFrom:{
			...
			handler(newVal,oldVal){
				transferOrders({'state':newVal.state,'beginTime':newVal.beginTime,'endTime':newVal.endTime}).then(res=>{
					this.tabledatas=res.data.data.records
				})
